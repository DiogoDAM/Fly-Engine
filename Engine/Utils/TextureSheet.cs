...
public class TextureSheet
{
	...
	{
		if(index < 0 || index >= Size) throw new IndexOutOfRangeException("FlyEngine :: TextureSheet.GetRegion() index is out of bounds");
	...
	public TextureRegion[] GetRegions(params int[] indexes)
	{
		...
		{
			int index = indexes[i];
			if(index < 0 || index >= Size) throw new IndexOutOfRangeException("FlyEngine :: TextureSheet.GetRegions() an index is out of bounds");
